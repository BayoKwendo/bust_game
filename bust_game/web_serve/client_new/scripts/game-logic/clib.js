define([
    ...
    function formatSatoshis(n, decimals) {
        return formatDecimals(n / 1, decimals);
    }
    ...
    return {
        ...
        parseBet: function (betString) {
            ...
            if (bet < 10)
                return new Error('The bet should be at least Ksh. 10');

            if (bet * 1 > AppConstants.Engine.MAX_BET)
                return new Error('The bet must be less no more than ' + 'Ksh. ' +formatSatoshis(AppConstants.Engine.MAX_BET) );
        ...
        parseAutoCash: function (autoCashString) {
            ...
            if (co < 1.01)
                return new Error('The auto cash out amount should be bigger than 1.01');
        ...
        profit: function (amount, cashOut) {
            ...
            var factor = Math.ceil(cashOut / amount);
        ...
        getElapsedTimeWithLag: function (engine) {
            if (engine.gameState == 'IN_PROGRESS') {
                var elapsed;
                if (engine.lag) {
                    elapsed = engine.lastGameTick - engine.startTime; //+ STOP_PREDICTING_LAPSE because it looks better
                }
                else {
                    // console.log("dd", engine.lastGameTick, engine.startTime)
