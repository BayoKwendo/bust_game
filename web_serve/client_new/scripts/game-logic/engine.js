define([
...
], function (io, Events, _, Clib, AppConstants, AppDispatcher) {
  function Engine() {
    var self = this;
    ...
    self.ws.on("game_started", function (bets) {
      ...
      Object.keys(bets).forEach(function (username) {
        if (self.username === username) self.balanceSatoshis -= bets[username];
    ...
    self.ws.on("check_user_info", function (resp) {
      self.balanceSatoshis = resp.balance_satoshis;
      // self.trigger('check_user_info');
    });
    ...
    self.ws.on("game_tick", function (elapsed) {
      ...
      self.tickTimer = setTimeout(
        self.checkForLag.bind(self),
    ...
    self.ws.on("game_crash", function (data) {
      ...
      if (data.game_crash == 0) self.setBonusesToZero();
    ...
    self.ws.on("player_bet", function (data) {
      console.log("Disbursed here", data);
      if (self.username === data.username) {
        self.placingBet = false;
        self.nextBetAmount = null;
        self.betAmount = data.betAmount;
        self.nextAutoCashout = null;
      }
    ...
    self.ws.on("cashed_out", function (resp) {
      ...
      console.log(
        "do cash",
        self.playerInfo[resp.username].bet + " " + resp.stopped_at
      );
      if (self.username === resp.username) {
        self.stopped_specific = resp.stopped_at;
        // if (self.gameState === 'ENDED') {
        //     if (resp.stopped_at > self.autoCashOut) {
        //         self.balanceSatoshis += self.playerInfo[resp.username].bet * resp.stopped_at;
        //     }
        // } else {
        self.balanceSatoshis +=
          self.playerInfo[resp.username].bet * resp.stopped_at;

        setTimeout((self.cashingOut = true), 1000);
        // self.cashingOut = true;
    ...
    self.ws.on("connect", function () {
      requestOtt(function (err, ott) {
        ...
        console.log("IS connected", ott);
        //If there is a Dev ott use it

        // self.ws.emit
        self.ws.emit("join", { ott: ott }, function (err, resp) {
          if (err) {
  ...
  Engine.prototype.bet = function (amount, autoCashOut, callback) {
    ...
    console.assert(
      !autoCashOut || (typeof autoCashOut === "number" && autoCashOut >= 2)
    );

    if (!Clib.isInteger(amount))
      return console.error(
        "The bet amount should be integer and divisible by 100" + amount
      );
  ...
  Engine.prototype.doBet = function (amount, autoCashOut, callback) {
    var self = this;
    // console.log("Place bet", autoCashOut)
  ...
  Engine.prototype.cancelBet = function () {
    if (!this.nextBetAmount)
      return console.error(
        "Can not cancel next bet, wasn't going to make it..."
      );
  ...
  Engine.prototype.cashOut = function () {
    ...
    console.log("cached out here againg");
  ...
  Engine.prototype.calcBonuses = function () {
    ...
    slideSameStoppedAt(
      ...
      function (array, listOfRecordsPositions, cashOutAmount, totalBetAmount) {
        ...
        if (bonusPool <= 0) {
          for (
            var i = 0, length = listOfRecordsPositions.length;
            i < length;
            i++
          ) {
        ...
        for (
          ...
          var toAlloc =
            (array[listOfRecordsPositions[i]].bet / totalBetAmount) *
            toAllocAll;
