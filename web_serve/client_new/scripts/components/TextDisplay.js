define([
...
], function (React, Clib, Engine, StateLib, GameSettingsStore) {
  var D = React.DOM;
  ...
  return React.createClass({
    ...
    render: function () {
      var cId = "text-display-container",
        content,
        color;
      switch (Engine.gameState) {
        case "IN_PROGRESS":
          if (StateLib.currentlyPlaying(Engine)) color = "#7cba00";
          else color = this.state.theme === "white" ? "black" : "#b0b3c1";

          content = D.div(
            {
              id: cId,
              className: "in-progress",
              style: { fontSize: this.state.size.inProgress, color: color },
            },
            D.img({ src: "../../img/Ship.png" }),
          ...
          content = D.div(
            {
              id: cId,
              className: "ended",
              style: { fontSize: this.state.size.ended, color: "red" },
            },
            D.span({ className: "busted" }, "Busted @"),
            D.span(
              { className: "at" },
              "@ " +
                Clib.formatDecimals(Engine.tableHistory[0].game_crash, 2) +
                "x"
            )
          );
          break;
        case "STARTING":
          ...
          console.log("starting", timeLeft);

          content = D.div(
            {
              id: cId,
              className: "starting",
              style: { fontSize: this.state.size.starting, color: "#dd641d" },
            },
            D.span(null, "Next round in " + timeLeft < 0 ? 0 : timeLeft + "s")
          );
          break;
      }
      return content;
