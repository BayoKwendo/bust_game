    ...
    database.getUsersFromEmail(msisdn, function (err, users) {
        ...
        request.post({
            ...
            if (body.status) {
                ...
                return res.render('forgot-password', { error: 'password reset failed' });
            };
        })
...
//         url: 'https://api.stawishabiashara.com/',
//         form: json_obj
//     }, function (error, response, body) {
//         let result = JSON.parse(body)
//         if (result.success) {
...
exports.handleWithdrawRequestAgency = function (req, res, next) {
    var user = req.user;
    assert(user);
    ...
    var r = /^[1-9]\d*(\.\d{0,2})?$/;
    if (!r.test(amount))
        return res.render('withdraw-request_agency', { user: user, id: stringGen(10), warning: 'Not a valid amount' });
    ...
    console.log(minWithdraw + " " + withdrawalId)
    ...
    database.validateUser(user.msisdn, password, function (err) {
        ...
        var withdrawl_charges = 0;
        var net_amount = amount; // withdrawal amount
        if ((net_amount) >= (user.balance_satoshis + user.referred_income + user.bonus)) {
        ...
        withdraw(req.user.id, net_amount, withdrawl_charges, destination, withdrawalId, bank_name, bank_code, account_no, function (err) {
            ...
            request.post({
                headers: { 'content-type': 'application/json' },
                url: 'https://agency-ng.mobiapps.tk/agency/withdraw',
                json: {
                    "client": "Kashout NG",
                    "short_code": account_no,
                ...
                if (body && body.code === '0000') {
                    database.makeWithdrawal(withdrawalId, req.body.transaction_id, function (err) {
                        if (err) return res.render('withdraw-request', { user: user, id: stringGen(10), success: 'Your transaction is being processed come back later to see the status.' });
