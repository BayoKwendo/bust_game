...
// function exciseTaxLogs(client, userId, amount, callback) {
//     client.query('INSERT INTO excise_duty_log(tax_amount, user_id) VALUES($1, $2)',
//         [amount, userId],
//         function (err, response) {
//             if (err) return callback(err);
//             callback(null);
...
// function withHoldingLogs(client, userId, amount, callback) {
...
// exports.updatefailed = function updatefailed(reference, description, callback) {

//     query('INSERT INTO fundings (status,description,bitcoin_deposit_txid) status = $1,  = $2 where  = $3',
//         ['failed', description, reference],
//         function (err, response) {
//             if (err) return callback(err);
//             // var fundingId = response.rows[0].id;
//             // assert(typeof fundingId === 'number');
...
exports.addDepositAgent = function (msisdn, transaction_id, amount, callback) {
    // assert(userId && callback);
    getClient(function (client, callback) {
        client.query('SELECT id FROM users WHERE msisdn = $1', [msisdn], function (err, result) {
            if (err) return callback(err);
            if (result.rows.length > 0) {
                var userId = result.rows[0].id
                addSatoshis(client, userId, amount, function (err) {
                    if (err) return callback(err);
                    makeDepositAgent(client, userId, amount, transaction_id, function (err) {
                        if (err) return callback(err);
                        callback(null);
                        // updateExciseDuty(client, tax_amount, userId, function (err) {
                        //     if (err) return callback(err);
                        // })
                    });
                });
