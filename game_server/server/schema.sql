...
-----------------+-----------------------------+-----------+----------+-------------------------------------
 id              | bigint                      |           | not null | nextval('bet_log_id_seq'::regclass)
 bet             | double precision            |           | not null | 
 cashout         | double precision            |           | not null | 0
 balance         | double precision            |           | not null | 0
 user_id         | bigint                      |           | not null | 
 narrative       | character varying           |           |          | 
 created         | timestamp with time zone    |           | not null | now()
 last_updated_on | timestamp without time zone |           | not null | now()

CREATE VIEW bet_log_view AS
 SELECT u.id,
    u.created,
    u.bet,
    u.cashout
