    ...
    function joined(socket, loggedIn) {
        ...
        socket.on('game_hash', function (offset, limit, ack) {
            ...
            socket.on('update_maxProfit', function (maxProfit, ack) {
                ...
                var nMaxProfit = game.maxWin + maxProfit;
        ...
        if (loggedIn)
            socket.on('place_bet', function (amount, autoCashOut, ack) { // Added the parameter 'autoCashOutChk'
                // console.log('autoCashOutChk', ack);
        ...
        socket.on('check_user_info', function (ack) {
            if (!loggedIn)
            ...
            console.log('bdbb', loggedIn)
            database.getBalance(loggedIn.id, function (err, user) {
                if (err) {
                    if (err == 'NOT_VALID_TOKEN')
                        return ack(err);
                    return internalError(socket, err, 'Unable to validate ott');
